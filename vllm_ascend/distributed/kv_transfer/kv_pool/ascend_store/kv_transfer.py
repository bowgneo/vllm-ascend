...
class KVTransferThread(threading.Thread):
    ...
    def _batch_copy_with_limits(
        ...
        for start in range(0, len(gvas), max_transfer_addrs):
            ...
            if res != 0:
                logger.error(
                    "event=kv_pool_batch_copy_failed operation=%s "
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                self._fatal_error = e
                logger.error(
                    "event=kv_pool_transfer_thread_failed task_id=%s "
    ...
    def lookup(
        ...
        try:
            ...
            logger.exception(
                "event=kv_pool_exists_failed operation=exists "
    ...
    def _handle_request(self, req_meta: ReqMeta):
        if self.worker is not None and getattr(self.worker, "tp_mismatch", False):
            ...
            try:
                ...
                logger.exception(
                    "event=kv_pool_save_failed req_id=%s "
    ...
    def _handle_request(self, req_meta: ReqMeta):
        try:
            ...
            if load_spec is None:
                logger.error(
                    "event=kv_pool_load_spec_missing req_id=%s "
...
class KVCacheStoreKeyLayerRecvingThread(KVTransferThread):
    ...
    def _wait_for_save(self, layer_id: int) -> None:
        while not self.layer_save_finished_events[layer_id].wait(timeout=10):
            logger.info(
                "event=kv_pool_layer_save_waiting layer_id=%d",
                layer_id,
            )
    ...
    def _handle_request(  # type: ignore[override]
        ...
        if data.attention_start_gate is not None:
            while not data.attention_start_gate.wait(timeout=10):
                logger.info(
                    "event=kv_pool_attention_start_waiting layer_id=%d",
                    layer_id,
                )
