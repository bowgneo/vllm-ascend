...
class MooncakeBackend(Backend):
    ...
    def put(self, keys: list[str], addrs: list[list[int]], sizes: list[list[int]]):
        ...
        try:
            ...
            if failed_count:
                ...
                logger.error(
                    "event=kv_pool_backend_put_failed backend=mooncake "
                ...
                logger.debug("event=kv_pool_backend_put_result key_count=%d result_count=%d", len(keys), len(res))
                logger.debug("event=kv_pool_backend_put_keys sample_keys=%s", keys[:3])
                if self._lazy_init:
    ...
    def get(self, keys: list[str], addrs: list[list[int]], sizes: list[list[int]]):
        if self._lazy_init and not self._store_initialized:
            logger.error(
                "event=kv_pool_backend_get_failed backend=mooncake "
        ...
        assert self.store is not None
        logger.debug(
            "event=kv_pool_backend_get_started backend=mooncake key_count=%d",
            len(keys),
        )
        try:
            ...
            if failed_count:
                ...
                logger.debug("event=kv_pool_backend_get_keys sample_keys=%s", keys[:3])
