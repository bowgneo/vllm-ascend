...
class MemcacheBackend(Backend):
    ...
    def _setup_store(self):
        ...
        logger.info(
            "event=kv_pool_backend_init_started backend=memcache rank_id=%d",
            self.local_rank,
        )
        try:
        ...
        try:
            ...
            logger.error(
                "event=kv_pool_backend_init_failed backend=memcache rank_id=%d "
        ...
        logger.info(
            "event=kv_pool_backend_init_succeeded backend=memcache rank_id=%d "
            "duration_ms=%.3f",
            self.local_rank,
            (time.perf_counter() - started_at) * 1e3,
        )
    ...
    def get(self, key: list[str], addr: list[list[int]], size: list[list[int]]):
        if self._lazy_init and not self._store_initialized:
            logger.error(
                "event=kv_pool_backend_get_failed backend=memcache "
        ...
        try:
            ...
            if failed_count:
                ...
                logger.debug("event=kv_pool_backend_get_keys sample_keys=%s", key[:3])
            return res
    ...
    def put(self, key: list[str], addr: list[list[int]], size: list[list[int]]):
        ...
        try:
            ...
            if failed_count:
                error_codes = sorted(set(failed_codes))
                logger.error(
                    "event=kv_pool_backend_put_failed backend=memcache "
                    "failed_keys=%d key_count=%d error_codes=%s reason=backend_returned_error "
                ...
                logger.debug("event=kv_pool_backend_put_keys sample_keys=%s", key[:3])
                if self._lazy_init:
