...
class KVPoolScheduler:
    ...
    def _get_layerwise_gva_hit_tokens(
        ...
        for group_id in range(len(self.grouped_block_size)):
            ...
            if len(key_infos) != len(all_keys):
                logger.error(
                    "event=kv_pool_lookup_result_invalid backend=%s "
    ...
    def get_num_new_matched_tokens(
        ...
        logger.info(
            "event=kv_pool_lookup_completed total_tokens=%d "
        ...
        logger.info(
            "event=kv_pool_load_spec_created vllm_cached_tokens=%d "
    ...
    def update_connector_output(self, connector_output: KVConnectorOutput):
        ...
        for event_id, count in meta.completed_events.items():
            ...
            if total == -1:
                logger.warning(
                    "event=kv_pool_worker_event_invalid event_id=%s count=%s "
